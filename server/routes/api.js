...
router.post("/signUp", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        conn.query(
            "SELECT * FROM users_superadmin WHERE email=?", [req.body.email],
...
router.post("/updateTask", function(req, res, next) {
    ...
    connection.getConnection(function(err, conn) {
        ...
        console.log(data);
        conn.query(
            "update tasks SET ? where id = '" + data.id + "'", [data],
            function(err, rows) {
                conn.release();
                if (!err) {
                    if (!err) {
    ...
    try {
        ...
        connection.getConnection(function(err, conn) {
            if (err) {
                ...
                conn.query(
                    "SELECT * FROM users WHERE email=? AND password=?", [reqObj.email, sha1(reqObj.password)],
                    function(err, rows, fields) {
                        if (err) {
                            console.error("SQL error:", err);
                            res.json({
                        ...
                        if (rows.length >= 1 && rows[0].active === 1) {
                            ...
                            conn.query(
                                "SELECT * FROM users_superadmin WHERE email=? AND password=?", [reqObj.email, sha1(reqObj.password)],
                                function(err, rows, fields) {
                                    if (err) {
                                        console.error("SQL error:", err);
                                        res.json({
...
router.post("/createUser", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        conn.query(
            "SELECT * FROM users WHERE email=? and superadmin=?", [req.body.email, req.body.superadmin],
            function(err, rows, fields) {
                if (err) {
                    console.error("SQL error:", err);
                    res.json({
...
router.get("/getUsers/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        console.log(id);
        conn.query(
            "SELECT u.id, u.shortname, u.firstname, u.lastname, u.email, u.street, u.active from users u join store s on u.storeId = s.id where s.superadmin = ?", [id],
            function(err, rows) {
                conn.release();
                console.log(rows);
                if (!err) {
...
router.get("/getUsersInCompany/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from users where storeId = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
    ...
    connection.getConnection(function(err, conn) {
        ...
        conn.query("SELECT * from users where id = ?", [id], function(err, rows) {
            if (!err && rows.length >= 1) {
                ...
                conn.query(
                    "SELECT * from users_superadmin where id = ?", [id],
                    function(err, rows) {
                        conn.release();
                        if (!err) {
                            res.json(rows);
...
router.post("/createStore", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        conn.query(
            "SELECT * FROM store WHERE email=? and superadmin=?", [req.body.email, req.body.superadmin],
            function(err, rows, fields) {
                if (err) {
                    console.error("SQL error:", err);
                    res.json({
...
router.get("/getStore/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from store where superadmin = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getDocuments/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from documents  where customer_id = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.post("/updateDocument", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        conn.query(
            "UPDATE documents SET ? where id = '" + id + "'", [req.body],
            function(err, rows) {
                conn.release();
                if (!err) {
                    if (!err) {
...
router.post("/uploadImage", function(req, res, next) {
    ...
    connection.getConnection(function(err, conn) {
        ...
        conn.query(
            "UPDATE users SET img = ? where id = '" + id + "'", [img],
            function(err, rows) {
                conn.release();
                if (!err) {
                    if (!err) {
...
router.post("/updateCustomer", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        conn.query(
            "UPDATE customers SET ? where id = '" + id + "'", [req.body],
            function(err, rows) {
                conn.release();
                if (!err) {
                    if (!err) {
    ...
    try {
        ...
        connection.getConnection(function(err, conn) {
            if (err) {
                ...
                conn.query(
                    "SELECT * FROM users u WHERE u.email=?", [reqObj.email],
                    function(err, rows, fields) {
                        if (err) {
                            console.error("SQL error:", err);
                            res.json({
                        ...
                        if (rows.length >= 1 && rows[0].active == "1") {
                            ...
                            conn.query(
                                "SELECT * FROM users_superadmin u WHERE u.email=?", [reqObj.email],
                                function(err, rows, fields) {
                                    conn.release();
                                    if (err) {
                                        console.error("SQL error:", err);
...
router.get("/getComplaintForCustomer/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from complaint where customer_id = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getTherapyForCustomer/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from therapy where customer_id = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getTherapy/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from therapy where id = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getVauchers/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from vaucher where superadmin = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getEventCategory/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "select * from event_category where superadmin = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getWorkTimeColors/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "select * from work_time_colors where superadmin = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.post("/updateToDo", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        console.log(req.body);
        conn.query(
            "update todo SET ? where id = '" + req.body.id + "'", [req.body],
            function(err, rows) {
                conn.release();
                if (!err) {
                    if (!err) {
